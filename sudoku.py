...
squaredomain = Domain(digits1to9)
...
def generate_sudoku_constraints():
    ...
    #blocks
    scs.append( Constraint([[str(br+1+(3*by))+str(bc+1+(3*bx)) \
                             for br in range(3) for bc in range(3)] \
                            for bx in range(3) for by in range(3)], 1) )  
...
def initialize_sudoku_solver( boardstring ):
    
    # initialize a solver with 81 variables '11','12',...,'98','99'
    # each with corresponding domain {1,...,9}
    ss = Solver( variables=dict([(v, squaredomain.copy()) for v in sudokuvars]) )

    # add the generic sudoku constraints to the solver
    ss.addConstraints( generate_sudoku_constraints() )
